...
def connected_modules(active_node):
    if active_node in list(module_data.df.index):
        ...
        depends_on_knowledge_in = str(module_data.df.loc[active_node, "depends_on_knowledge_in"])
        hidden_button_list = []
        ...
        for module in list(module_data.df.index):
            if module in sets_you_up_for or active_node in str(module_data.df.loc[module,"depends_on_knowledge_in"]): #ensure that if the link is only in one of the modules, it still shows up here (symmetry in metadata not required)
            ...
            elif module in depends_on_knowledge_in or active_node in str(module_data.df.loc[module,"sets_you_up_for"]):
