...
### Use ctx to determine whether the last thing clicked was a button or a node on the graph, 
### then make that thing the ACTIVE NODE
def active_node_in(app):
    ...
    def activate(current_active_node,data, *args):
        ...
        # Was this triggered by clicking on a node in the graph?
        if trigger == "module_visualization":
            return data['id']
        elif sum(args) != 0:
             return trigger[:-7] # why -7?  is this an error code?
        else:
